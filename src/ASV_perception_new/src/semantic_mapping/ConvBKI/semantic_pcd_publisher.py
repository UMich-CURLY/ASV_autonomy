...
def image_msg_to_numpy(img_msg, desired_encoding=None):
    """Convert common uint8 ROS Image encodings without cv_bridge.

    This avoids Conda/system OpenCV library conflicts in cv_bridge for the
    simple mono and color image formats used by this node.
    """
    encoding = img_msg.encoding.lower()
    desired = desired_encoding.lower() if desired_encoding else encoding

    if encoding in ("mono8", "8uc1"):
        arr = np.frombuffer(img_msg.data, dtype=np.uint8)
        arr = arr.reshape(img_msg.height, img_msg.step)
        arr = arr[:, :img_msg.width].copy()
        if desired in ("mono8", "8uc1", "passthrough"):
            return arr
        if desired == "bgr8":
    ...
    if encoding in ("bgr8", "rgb8"):
        arr = np.frombuffer(img_msg.data, dtype=np.uint8)
        arr = arr.reshape(img_msg.height, img_msg.step)
        arr = arr[:, :img_msg.width * 3].reshape(img_msg.height, img_msg.width, 3).copy()
        if desired in (encoding, "passthrough"):
            return arr
        if desired == "bgr8" and encoding == "rgb8":
            return cv2.cvtColor(arr, cv2.COLOR_RGB2BGR)
        if desired == "rgb8" and encoding == "bgr8":
            return cv2.cvtColor(arr, cv2.COLOR_BGR2RGB)

    if encoding in ("bgra8", "rgba8"):
        arr = np.frombuffer(img_msg.data, dtype=np.uint8)
        arr = arr.reshape(img_msg.height, img_msg.step)
        arr = arr[:, :img_msg.width * 4].reshape(img_msg.height, img_msg.width, 4).copy()
        if desired in (encoding, "passthrough"):
            return arr
        if desired == "bgr8":
    ...
    raise ValueError(
        f"Unsupported image conversion from encoding '{img_msg.encoding}' "
        f"to '{desired_encoding or img_msg.encoding}'"
    )


def numpy_to_image_msg(img, header, encoding):
    msg = Image()
    msg.header = header
    ...
    msg.step = msg.width * channels * img.dtype.itemsize
...
class MapPublisher(Node):
    ...
    def __init__(self, model_params, e2e_net, dev, dtype):
        ...
        self.publish_overlay = model_params["ConvBKI"].get("publish_overlay", False)
        self.lidar = None
        self.seg_input = None
        self.inv = None
    ...
    def create_point_labels(self, mask_msg):        
        ...
        label_mask = image_msg_to_numpy(mask_msg, desired_encoding="mono8")
    ...
    def publish_overlay_image(self, annot_msg):
        ...
        annot_img = image_msg_to_numpy(annot_msg, desired_encoding="bgr8")
        ...
        overlay_msg = numpy_to_image_msg(annot_img, annot_msg.header, encoding="bgr8")
    ...
    def callback(self, filt_pcd_msg, pose_msg, mask_msg, annot_msg):
        ...
        try:
            ...
            self.get_logger().warning(f"Skipping debug overlay image: {exc}")
        ...
        try:
            ...
            self.get_logger().error(f"Could not convert segmentation mask: {exc}")
            return
