...
def load_model(model_params, dev, logger=None):
    # The active Blueboat perception path uses Grounded-SAM2 labels directly.
    # Keep the BKI model lean and avoid pulling in the legacy SPVCNN/TorchSparse
    # dependency chain unless we intentionally revive that path.
    seg_net = None

    _log(logger, "info", f"Building ConvBKI runtime on {dev}.")
    ...
    f = model_params["f"]
    _log(logger, "info", f"ConvBKI config: num_classes={num_classes}, filter_size={f}.")
    ...
    _log(logger, "info", "ConvBKI runtime ready.")
    return e2e_net
