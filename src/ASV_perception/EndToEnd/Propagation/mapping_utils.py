...
def invert_rigid_transform(transform):
    """Invert a 4x4 SE(3) transform without torch.linalg.inv.

    This avoids dispatching tiny pose inversions through CUDA linear algebra
    kernels, which are unnecessary here and can be brittle across Jetson
    ...
    inverse[:3, 3] = -torch.matmul(rotation_t, translation)
    inverse[3, 3] = 1
    return inverse
...
class TransformWorldStatic(torch.nn.Module):
    ...
    def forward(self, new_pose, current_map):
        ...
        initial_to_world = invert_rigid_transform(self.initial_pose)
        prev_to_initial = torch.matmul(initial_to_world, prev_pose)
        prev_translation = prev_to_initial[:3, 3]
        prev_voxel = torch.round(prev_translation / self.voxel_sizes)

        new_to_initial = torch.matmul(initial_to_world, new_pose)
